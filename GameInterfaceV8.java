...
public class GameInterfaceV8 extends JFrame implements ActionListener {
	...
	public GameInterfaceV8(SBoardV8 b) {//Unknown
	...
	public GameInterfaceV8() {//Unknown
	...
	public GameInterfaceV8(ShoSquareBox[][] board) {//unknown
		this.setBoard(board);
		initUI();
		this.setVisible(true);
	}
	...
	public void initUI() {
		...
		for (int i = 0; i < this.board.length; i++) {
			for (int j = 0; j < this.board[i].length; j++) {
				GameInterfaceV8.buttons[i][j] = new ShoSquareBox(" ", i, j, boardImportant.getValue(i, j));
				GameInterfaceV8.buttons[i][j].addActionListener(new ActionListener()
				{
					public void actionPerformed(ActionEvent e)
					{
						...
						//boardImportant.CapturePiece2(row, column);
						/*if(tempValue == 3) {
...
public static void main(String[] args) {
	SwingUtilities.invokeLater(new Runnable() {
		public void run() {
			GameInterfaceV8 q = new GameInterfaceV8();
			q.boardImportant.possibleMoves2();
